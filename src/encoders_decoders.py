...
class CategoricalEncoder(torch.nn.Module):
    ...
    def forward(self,x):
        # x has shape [batch_size, xdim], a,b,c has shape [xdim, N]
...
class BernoulliEncoder(torch.nn.Module):
    ...
    def forward(self,x):
        # x has shape [batch_dim,x_dim], c,log_sigma,A have shape [x_dim, N]
...
class MoGDecoder(torch.nn.Module):
    ...
    def forward(self,r):
        # r is a tensor [batch_dim,N],mus and sigmas are [N,1]. Note that input 
...
class GaussianDecoder(torch.nn.Module):
    ...
    def forward(self,r):
        # r must have shape [nbatch,N], phi have shape [N,2]
